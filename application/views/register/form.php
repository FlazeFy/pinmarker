...
<div>
    ...
    <div style="width: 720px;" class="d-block mx-auto mt-4">
        <form method="POST" action="/RegisterController/register">
            ...
            <div style="height:90vh; padding-top:calc(var(--spaceJumbo)*3); display:none;" id="validate_holder"></div>
        </form>
    </div>
</div>
...
<script>
    ...
    function navTerms(){
        ...
        $('#user_profile_holder').html(`
            ...
            <a class="btn btn-dark rounded-pill px-3" onclick="navProfile()" disabled><i class="fa-solid fa-arrow-right-to-bracket me-2"></i> Create Account</a>
        `)
    }
    ...
    function navProfile(){
        if(validateInput("text", "username", 36, 4) && validateInput("text", "email", 255, 10) && validateInput("text", "password", 36, 6) && validateInput("text", "password_check", 36, 6)){
            ...
            .done(function (response) {            
                ...
                const data = response
                $(document).ready(function() {
                    $('#checkTerm, #username, #fullname, #email, #password, #password_check').attr('disabled', true)
                    $('#profile_indicator').attr('class', 'active')
                    $('#validate_holder').css({'display':'block'})
                });
                $('#validate_holder').html(`
